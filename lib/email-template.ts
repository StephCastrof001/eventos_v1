...
export function buildPendingEmail(
	...
	if (eventDate || eventLocation || instructions) {
		...
		const locationHtml = eventLocation
			? locationUrl
				? `<a href="${escapeHtml(locationUrl)}" target="_blank" style="color: #6f5ff2; text-decoration: underline;">📍 <strong>${escapeHtml(eventLocation)}</strong></a>`
				: `<p style="margin: 0; font-size: 14px; color: #e8e8f0;">📍 <strong>${escapeHtml(eventLocation)}</strong></p>`
			: "";
		...
		const instructionsHtml = instructions
			? `<div style="margin-top: 12px; padding: 12px; background-color: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 6px; font-size: 13px; color: #e8e8f0;"><strong>ℹ️ Instrucciones:</strong><br/>${escapeHtml(instructions)}</div>`
			: "";

		detailsHtml = `
		<div style="margin-top: 24px; padding: 16px; background-color: #0c0c14; border-radius: 6px; text-align: left;">
			${dateStr ? `<p style="margin: 0 0 8px 0; font-size: 14px; color: #e8e8f0;">📅 <strong>${dateStr}</strong></p>` : ""}
